...
public class DetailItemActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (from.equals(VAL_MOVIE)){
            detailViewModel.getFavMovie().observe(this, getFavMovie);
        }else {
            detailViewModel.getFavTv().observe(this, getFavTv);
        }
    }
    ...
    private Observer<MovieEntity>getFavMovie = new Observer<MovieEntity>() {
        @Override
        public void onChanged(MovieEntity movieEntity) {
            onDb = movieEntity != null;
            favoriteAction();
        }
    };

    private Observer<TvEntity>getFavTv = new Observer<TvEntity>() {
        @Override
    ...
    private void favoriteAction(){
        Log.i("k", "favoriteAction: "+onDb);
        if (onDb){
            fab.setImageResource(R.drawable.ic_favorite_clicked);
        }else {
            ...
            public void onClick(View view) {
                if (!onDb){
                    if (from.equals(VAL_MOVIE)){
                        ...
                        detailViewModel.insertTv();
                    }
                    Toast.makeText(DetailItemActivity.this, "Ditambahkan ke favorite", Toast.LENGTH_SHORT).show();
                }else {
                    if (from.equals(VAL_MOVIE)){
                        detailViewModel.deleteMovie();
                    }else {
                        detailViewModel.deleteTv();
                    }
                    finish();
                    Toast.makeText(DetailItemActivity.this, "Dihapus dari favorite", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
